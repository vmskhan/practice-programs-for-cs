        ...
        if(curNodeColour==0)
        {
          ...
          //there will always be a black/red sibling for a black node, no need to check for null
            ...
            if(ncColour==0 && fcColour==0)
            {
              ...
              //apply case -6
              //recalculation befroe case-6 because of rotation  
              sibling=isRootLeft?parent->right:parent->left;
              nearChild=isRootLeft?sibling->left:sibling->right;
              farChild=isRootLeft?sibling->right:sibling->left;
              ncColour=nearChild==NULL?0:nearChild->colour;
              fcColour=farChild==NULL?0:farChild->colour;
